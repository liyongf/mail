...
public class CommonController extends BaseController {
	...
	public ModelAndView openViewFile(HttpServletRequest request) {
		...
		if ("dwg".equals(extend)) {
			...
			File swfFile = new File(localDir+swfpath);
			if (!swfFile.exists()) {
				String realpath = oConvertUtils.getString(reflectHelper.getMethodValue("realpath"));
				reflectHelper.setMethodValue("swfpath", FileUtils.getFilePrefix(realpath) + ".swf");
				realpath = localDir + realpath;
				SwfToolsUtil.convert2SWF(realpath);
			}
