...
//    }
	...
	public AjaxJson saveDepart(TSDepart depart, HttpServletRequest request) {
		...
		if (StringUtil.isNotEmpty(depart.getId())) {
			...
			if(StringUtils.isNotEmpty(sjdw)&&sjdw.equals("true")){
...
//			depart.setOrgCode(orgCode);
		...
		//更新部门信息
		systemService.initAllDepartCach();
		j.setMsg(message);
		return j;
	}
    ...
    public AjaxJson saveFiles(HttpServletRequest request, HttpServletResponse response, TSDocument document) {
        ...
        String docSourceId = request.getParameter("docSourceId");
        ...
        if(docSourceId != ""){
            TBDocumentSourceEntity tds = systemService.getEntity(TBDocumentSourceEntity.class,docSourceId);
            document.setDocumentSource(tds);
        }
       /* if(StringUtils.isNotBlank(docSourceId)){
            TBDocumentSourceEntity documentSource = systemService.getEntity(TBDocumentSourceEntity.class,docSourceId);
...
//				cq.eq("documentTitle", documentTitle);
				cq.like("documentTitle", "%"+documentTitle+"%");
			}
			String createTimeStart = request.getParameter("createdate_begin");
			String craeteTimeEnd = request.getParameter("createdate_end");
			if (StringUtils.isNotBlank(createTimeStart)) {
				SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
				cq.ge("createdate", sdf.parse(createTimeStart));
			}
			if (StringUtils.isNotBlank(craeteTimeEnd)) {
				SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
				cq.le("createdate", sdf.parse(craeteTimeEnd));
			}
		} catch (Exception e){
			e.printStackTrace();
    ...
    public void documentList(HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        ...
        if(StringUtils.isNotBlank(docSourceId)){
            TBDocumentSourceEntity documentSource = systemService.getEntity(TBDocumentSourceEntity.class,docSourceId);
            if(documentSource != null && !documentSource.getChildDocSources().isEmpty() && documentSource.getChildDocSources().size()>0){
                //获取所有子节点下的文件
                List<TBDocumentSourceEntity> childDocSourceList = documentSource.getChildDocSources();
                List<String> childDocSources = new ArrayList<String>();
                for(TBDocumentSourceEntity ds : childDocSourceList){
                    childDocSources.add(ds.getId());
                }
                cq.in("documentSource.id",childDocSources.toArray());
            }else{
                cq.eq("documentSource.id", docSourceId);
            }
        }
        cq.eq("status", 1);
        ...
        this.systemService.getDataGridReturn(cq, true);
        /*
        if("termsFile".equals(typecode)){
            if(dataGrid != null && dataGrid.getResults() != null){
                List<TSDocument> termsFileList = dataGrid.getResults();
                for(TSDocument doc : termsFileList){
                    StringBuffer sb = new StringBuffer();
                    sb.append("SELECT COUNT(1) AS count FROM t_b_danger_source ds WHERE 1=1 ").append(" AND ds.item_id IN ( ");
                    sb.append(" SELECT i.id FROM t_b_item i WHERE i.file_id = '").append(doc.getId()).append("') ");
                    List<BigInteger> countList = systemService.findListbySql(sb.toString());
                    if(!countList.isEmpty() && countList.size()>0){
                        String count = String.valueOf(countList.get(0));
                        doc.setCountDangerSrc(count);
                    }
                }
            }
        }*/
		...
		try{
			if(StringUtil.isNotEmpty(ids)){
				for (String id : ids.split(",")){
					...
					sunshineEntity.setTableName("t_s_document");
