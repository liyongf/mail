...
//		}
	...
	public StringBuffer end() {
		...
		sb.append("<script type=\"text/javascript\">");
		if (StringUtil.isEmpty(onDblClick)){
			// List<DataGridUrl>
			for(DataGridUrl dgu : toolBarList){
				if("查看".equals(dgu.getTitle())){
					String url = dgu.getUrl();

					sb.append("function "+name+"_dblClickDetail(rowIndex,rowData){");
					sb.append(" var id=rowData.id;");
					//TODO houbin 修改查看url地址，改为动态地址

					String controllerName = actionUrl.substring(0,actionUrl.indexOf(".do"));
					sb.append(" var url = \""+url+"&load=detail&id=\"+id;" );
					sb.append("createdetailwindow(\"查看\",url,950,400); ");
        ...
        if(StringUtils.isNotBlank(pageList)){
            ...
            sb.append("pageList:[" + pageSize * 1 + "," + pageSize * 2 + "," + pageSize * 3 + "," + pageSize * 5 + "," + pageSize * 10  + "," + pageSize * 50 + "," + pageSize * 100 + " ],");
        }
		...
		if (StringUtil.isNotEmpty(onDblClick)) {
			...
			//提供一个默认的双击事件
			sb.append("onDblClickRow:function(rowIndex,rowData){" + name + "_dblClickDetail" + "(rowIndex,rowData);},");
		}
		if (treegrid) {
	...
	protected void getFun(StringBuffer sb, DataGridColumn column) {
		...
		url = formatUrl(url);
		sb.append("var href=\"<a style=\'color:red\' href=\'#\' onclick=" + column.getFunname() + "('" + column.getTitle() + "','" + url + "','','1000px','450px')>\";");
