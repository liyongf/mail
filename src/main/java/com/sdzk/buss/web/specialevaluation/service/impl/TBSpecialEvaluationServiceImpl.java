package com.sdzk.buss.web.specialevaluation.service.impl;
...
@Service("tBSpecialEvaluationService")
@Transactional
public class TBSpecialEvaluationServiceImpl extends CommonServiceImpl implements TBSpecialEvaluationServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(TBSpecialEvaluationEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(TBSpecialEvaluationEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doDelBus(TBSpecialEvaluationEntity t) throws Exception{
	    //-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
 	...
 	private Map<String,Object> populationMap(TBSpecialEvaluationEntity t){
		Map<String,Object> map = new HashMap<String,Object>();
		map.put("id", t.getId());
		map.put("name", t.getName());
		map.put("leader", t.getLeader());
		map.put("leader_type", t.getLeaderType());
		map.put("time", t.getTime());
		map.put("participant", t.getParticipant());
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,TBSpecialEvaluationEntity t){
 		sql  = sql.replace("#{id}",String.valueOf(t.getId()));
 		sql  = sql.replace("#{name}",String.valueOf(t.getName()));
 		sql  = sql.replace("#{leader}",String.valueOf(t.getLeader()));
 		sql  = sql.replace("#{leader_type}",String.valueOf(t.getLeaderType()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("t_b_special_evaluation",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
