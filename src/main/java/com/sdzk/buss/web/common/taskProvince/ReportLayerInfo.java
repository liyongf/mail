 ...
 */
@Component("reportLayerInfo")
public class ReportLayerInfo {
    @Autowired
    ...
    public void run(){
        ...
        long count = 0;
        org.jeecgframework.core.util.LogUtil.info("===================定时上报煤层信息开始===================");//调整
        try {
            String sql = "select id from t_b_layer where report_date_province is null or update_date > report_date_province ";//调整
            ...
            org.jeecgframework.core.util.LogUtil.error("定时上报煤层信息失败", e);//调整
        }
        org.jeecgframework.core.util.LogUtil.info("===================定时上报煤层信息结束===================");//调整
        long end = System.currentTimeMillis();
        long times = end - start;
        org.jeecgframework.core.util.LogUtil.info("定时上报煤层信息总耗时"+times+"毫秒,共同步"+count+"条数据.");//调整
    }
    ...
    public AjaxJson reportDataToProvince(String ids,String reportBy){
        ...
        if(StringUtil.isNotEmpty(ids)){
            ...
            if(jsonArray.size()>0){
                String reportUrl = ResourceUtil.getConfigByName("layerInfoReportToProvince");//调整
                j = getDataService.postData(jsonArray,reportUrl);
                //更新本地库
                if(j.getObj().equals("200")){
                    StringBuffer sqlUpdate = new StringBuffer("update t_b_layer b set \n");//调整
                    ...
                    j.setMsg(retList.size()+"条煤层信息上报成功");//调整
                }else{
    ...
    private static Map<String,String> getColumnNeed(){//调整
        ...
        columnNeed.put("layer_code","layerCode");
        columnNeed.put("layer_detail_name","layerDetailName");
        columnNeed.put("url","url");
        columnNeed.put("center","center");
        columnNeed.put("is_show","isShow");
        return columnNeed;
    }

    private static String getTableNeed(){//调整
        String tableName = "t_b_layer b";
        return tableName;
    }
}
