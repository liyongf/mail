...
<div id="main_typegroup_list" class="easyui-layout" fit="true">
    <div region="center" style="padding:0px;border:0px">
        ...
        idField="id" treegrid="false" pagination="false"  sortOrder="desc" sortName="createDate" onLoadSuccess="loadSuccess" queryMode="group" onClick="showOrHideEditButton">
            ...
            <t:dgCol title="来源" replace="煤监局_1,矿_2" field="origin" width="50" ></t:dgCol>
            <t:dgCol title="common.operation" field="opt" width="100"></t:dgCol>
            <t:dgDelOpt url="systemController.do?delTypeGroup&id={id}" title="common.delete" exp="origin#ne#1" urlclass="ace_button" urlStyle="background-color:#ec4758;" urlfont="fa-trash-o"></t:dgDelOpt>
            <t:dgFunOpt funname="queryTypeValue(id,typegroupname,origin)" title="common.type.view" urlclass="ace_button"  urlfont="fa-search"></t:dgFunOpt>

            <t:dgToolBar title="common.add.param" operationCode="add" langArg="lang.dictionary.type" icon="icon-add" url="systemController.do?aouTypeGroup" funname="add"></t:dgToolBar>
            <%--<t:dgToolBar title="common.add.param" langArg="lang.dictionary.value" icon="icon-add" funname="typeGridTree_AddType"></t:dgToolBar>--%>
            <t:dgToolBar title="common.edit" operationCode="update" icon="icon-edit" url="systemController.do?aouTypeGroup" funname="update"></t:dgToolBar>
            <t:dgToolBar title="同步" operationCode="synchronize" icon="icon-reload"  funname="updateDataDic"></t:dgToolBar>
            <t:dgToolBar title="刷新"  icon="icon-reload"  funname="refleshAllTypeGroups" operationCode="refleshAll" ></t:dgToolBar>
        </t:datagrid>
    </div>
</div>
...
<script type="text/javascript">
    ...
    function queryTypeValue(typegroupid, typegroupname, origin){
        ...
        $('#userListpanel').panel("refresh", "systemController.do?goTypeGrid&typegroupid=" + typegroupid+"&origin="+origin);
    }
    ...
    function showOrHideEditButton(rowIndex,rowData) {
        if (rowData.origin == '1'){
            $("#main_typegroup_list [icon='icon-edit']").hide();
        } else {
            $("#main_typegroup_list [icon='icon-edit']").show();
        }
    }
    ...
    function updateDataDic(){
        ...
        $.ajax({
            url : 'systemController.do?updateDataDic',
            type : 'post',
            cache : false,
            ...
            complete: function(date){
                ...
                jQuery('#typeGridTree').datagrid('reload');
                jQuery('#typeGridTree').treegrid('reload');
            },
            error:function(data){
    ...
    function refleshAllTypeGroups(){
        releshing();

        $.ajax({
            url : 'systemController.do?reFleshAllTypeGroup',
            type : 'post',
            cache : false,
            success:function(data){
                var d = $.parseJSON(data);
