...
<!------------------------------- -------------------------------------------------->
        function choose_dangerSource(title){
            if (typeof(windowapi) == 'undefined') {
                $.dialog({content: 'url:tBDangerSourceController.do?chooseDangerSource', zIndex: 2100, title: ''+title+'', lock: true, width: 900, height: 450, left: '85%', top: '65%', opacity: 0.4, button: [
                    {name: '<t:mutiLang langKey="common.confirm"/>', callback: clickcallback_dangerSource, focus: true},
                    {name: '<t:mutiLang langKey="common.cancel"/>', callback: function () {
                    }}
                ]});
            } else {
                $.dialog({content: 'url:tBDangerSourceController.do?chooseDangerSource', zIndex: 2100, title: ''+title+'', lock: true, parent: windowapi, width: 900, height: 450, left: '85%', top: '65%', opacity: 0.4, button: [
                    {name: '<t:mutiLang langKey="common.confirm"/>', callback: clickcallback_dangerSource, focus: true},
                    {name: '<t:mutiLang langKey="common.cancel"/>', callback: function () {
                    }}
                ]});
        ...
        function clickcallback_dangerSource(){
          //  debugger
            ...
            var dangerName = iframe.gettBDangerSourceListSelections('yeMhazardDesc');
            $("#dangerName").text(dangerName);
            $("#dangerName").blur();
...
<t:formvalid formid="formobj" dialog="false" layout="div" callback="test" action="jeecgFormvalidController.do?testsubmit=2" beforeSubmit="setContentc">
	 ...
	 <t:hasPermission code="phone_code">
		...
		<div class="form">
         ...
         <div class="form">
             <label class="Validform_label"> 选择危险源： </label>
             <input id="dangerId.id" name="dangerId.id" type="hidden" style="width: 150px" class="inputxt" >
             <textarea id="dangerName" name="dangerName" style="width: 440px;" datatype="*" onclick="choose_dangerSource('选择危险源');"></textarea>
             <span class="Validform_checktip"></span>
         </div>
