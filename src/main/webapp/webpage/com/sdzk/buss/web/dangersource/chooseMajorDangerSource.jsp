...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="tBDangerSourceListMajor" checkbox="true" fitColumns="false" title="危险源列表" actionUrl="tBDangerSourceController.do?initChooseMajorGeneralDanger" idField="id" fit="true" queryMode="group"  onLoadSuccess="loadSuccess">
 ...
 $(document).ready(function(){
     $('#formtree').tree({
         ...
         onClick : function(node) {
             ...
             var ids='';
             var param=$('#tBDangerSourceListMajor').datagrid('options').queryParams;
             ...
             param["ids"]=ids;
             $('#tBDangerSourceListMajor').datagrid('reload',param);
         }
     });
     $("#ttBDangerSourceListMajorForm").find("input[name='yeRecognizeYear']").attr("class", "Wdate").click(function () {
         WdatePicker({
             dateFmt: 'yyyy'
         });
     });
    ...
    api.button(
            {
                ...
                callback: function(){
                    var ids = [];
                    var rows = $('#tBDangerSourceListMajor').datagrid('getSelections');
                    if(rows.length <=0){
                        api.close();
                    }else{
                        ...
                        ids.join(',');
                        $.ajax({
                            url: "tBDangerSourceController.do?saveChooseMajorGeneralDanger&ids="+ids+"&tt="+new Date(),
                            type: 'POST',
