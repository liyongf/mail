...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="tBHiddenDangerExamList" onDblClick="dblClickDetail" checkbox="true" fitColumns="false" title="隐患检查" actionUrl="tBHiddenDangerExamController.do?datagrid&examType=${examType}" idField="id" fit="true" queryMode="group" sortName="examDate" sortOrder="desc">
   <t:dgCol title="主键"  field="id"  hidden="true"  queryMode="group"  sortable="false" width="120" align="center"></t:dgCol>
   <t:dgCol title="日期"  field="examDate"  formatter="yyyy-MM-dd" query="true"  queryMode="group"  sortable="false" width="100" align="center"></t:dgCol>
   <c:if test="${examType ne 'yhpc'}">
      <t:dgCol title="班次"  field="shift" hidden="false" dictionary="workShift" queryMode="single"  sortable="false" width="80" align="center"></t:dgCol>
   </c:if>
   <t:dgCol title="地点"  field="address.address"    queryMode="group"  sortable="false" width="120" align="center"></t:dgCol>
   <t:dgCol title="检查人"  field="fillCardManNames"    queryMode="group"  sortable="false" width="120" align="center"></t:dgCol>
   <c:if test="${examType eq 'other'}">
       <t:dgCol title="检查类型"  field="checkType"    queryMode="group"  sortable="false" width="120" align="center"></t:dgCol>
   </c:if>

   <%--新增关键字部分--%>
   <%--<t:dgCol title="关键字"  field="keyWords" queryMode="group" sortable="false" width="250" align="center"></t:dgCol>--%>

   <t:dgCol title="责任单位"  field="dutyUnit.departname"    queryMode="group"  sortable="false" width="120" align="center"></t:dgCol>
   <t:dgCol title="责任人"  field="dutyMan"    queryMode="group"  sortable="false" width="120" align="center"></t:dgCol>
      <t:dgCol title="危险源"  field="dangerId.hazard.hazardName"   queryMode="single"  sortable="false" width="120" align="center"></t:dgCol>
      <t:dgCol title="问题描述"  field="problemDesc"    queryMode="group"  sortable="false" width="120" formatterjs="valueTitle"></t:dgCol>
   <t:dgCol title="隐患类别"  field="hiddenCategory"  hidden="true"  queryMode="group"  sortable="false" width="120" align="center"></t:dgCol>
   <t:dgCol title="隐患等级"  field="hiddenNature" hidden="true"       queryMode="group"  sortable="false" width="120" align="center"></t:dgCol>
   <t:dgCol title="下井时间开始"  field="beginWellDate" hidden="true" formatter="yyyy-MM-dd"   queryMode="group"  sortable="false" width="120" align="center"></t:dgCol>
   <t:dgCol title="下井时间结束"  field="endWellDate" hidden="true" formatter="yyyy-MM-dd"   queryMode="group"  sortable="false" width="120" align="center"></t:dgCol>
   <t:dgCol title="经过路线"  field="routeLine" hidden="true"   queryMode="group"  sortable="false" width="120"></t:dgCol>
      <c:if test="${examType ne 'yhpc'}">
   <t:dgCol title="处理类型"  field="dealType" replace="限期整改_1,现场处理_2"  queryMode="group"  sortable="false" width="120" align="center"></t:dgCol>
      </c:if>
   <t:dgCol title="处理状态"  field="handleEntity.handlelStatus" dictionary="handelStatus"  queryMode="group"  sortable="false" width="120" align="center"></t:dgCol>
      <c:if test="${examType ne 'yhpc'}">
   <t:dgCol title="限期日期"  field="limitDate" formatter="yyyy-MM-dd"   queryMode="group"  sortable="false" width="120" align="center"></t:dgCol>
      <t:dgCol title="限期班次"  field="limitShift" hidden="true" dictionary="t_b_shift_manage,id,shift_name,where is_used = 1" queryMode="single"  sortable="false" width="120" align="center"></t:dgCol>
      </c:if>
   ...
   <t:dgCol title="驳回备注"  field="rollBackRemarkTemp"  hidden="false" query="false" queryMode="group"  sortable="false" width="300" formatterjs="valueTitle"></t:dgCol>
   ...
   <t:dgToolBar  title="录入" icon="icon-add" url="tBHiddenDangerExamController.do?goAdd&examType=${examType}" funname="goAdd"  width="950" height="400" operationCode="add"></t:dgToolBar>
      <t:dgToolBar title="查看" icon="icon-search" url="tBHiddenDangerExamController.do?goUpdate&examType=${examType}" funname="detail" width="950" height="600" operationCode="detail"></t:dgToolBar>
   <t:dgToolBar title="编辑" icon="icon-edit" url="tBHiddenDangerExamController.do?goUpdate&examType=${examType}" funname="gotoUpdate" width="950" height="400" operationCode="update"></t:dgToolBar>
   <t:dgToolBar title="批量删除"  icon="icon-remove" url="tBHiddenDangerExamController.do?doBatchDel" funname="deleteALLSelect" operationCode="batchdelete"></t:dgToolBar>
      ...
      <t:dgToolBar  title="查看罚款" icon="icon-add" url="tBHiddenDangerExamController.do?goLinkFine" funname="goLinkFine"  width="950" height="400" operationCode="goLinkFine"></t:dgToolBar>
  </t:datagrid>
  </div>
 </div>
 ...
 <script type="text/javascript">
     ...
     function goAdd(){
         addOneTab("<%=examTypeName%>-录入","tBHiddenDangerExamController.do?goAdd&examType=${examType}","default");
     }
     function gotoUpdate(){
         //获取选中条目
         var rowsData = $('#tBHiddenDangerExamList').datagrid('getSelections');
...
//导入
function ImportXls() {
	openuploadwin('Excel导入', 'tBHiddenDangerExamController.do?upload&examType=${examType}', "tBHiddenDangerExamList");
}
...
//导出
     function ExportXls() {
         ...
         if(rows==0){
             JeecgExcelExport("tBHiddenDangerExamController.do?exportXls&examType=${examType}","tBHiddenDangerExamList");
             ...
             $.dialog.confirm("是否确认导出"+idsTemp.length+"条记录？", function () {
                 JeecgExcelExport("tBHiddenDangerExamController.do?exportXls&examType=${examType}&ids="+idt,"tBHiddenDangerExamList");
             });
         }
