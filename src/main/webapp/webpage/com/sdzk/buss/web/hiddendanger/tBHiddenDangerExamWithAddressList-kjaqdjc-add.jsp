...
function choose_fillCardMan(title) {
    if (typeof(windowapi) == 'undefined') {
        $.dialog({content: 'url:userController.do?userManySelectList', zIndex: 2100, title: '' + title + '', lock: true, width: 860, height: 480, left: '85%', top: '65%', opacity: 0.4, button: [
            {name: '<t:mutiLang langKey="common.confirm"/>', callback: clickcallback_fillCardMan, focus: true},
            {name: '<t:mutiLang langKey="common.cancel"/>', callback: function () {
            }}
        ]});
    } else {
        $.dialog({content: 'url:userController.do?userManySelectList', zIndex: 2100, title: '' + title + '', lock: true, parent: windowapi, width: 860, height: 480, left: '85%', top: '65%', opacity: 0.4, button: [
            {name: '<t:mutiLang langKey="common.confirm"/>', callback: clickcallback_fillCardMan, focus: true},
            {name: '<t:mutiLang langKey="common.cancel"/>', callback: function () {
            }}
        ]});
...
//        choosed = choosed + "," + realName;     //这是原版代码

        //改     Author：张赛超
        var flag = 0;           //flag=0代表无重复组员
        var temp = Array();
        temp = choosed.split(',');
        for(var i=0; i<temp.length; i++){
            if(realName == temp[i]){
                flag = 1;       //flag=1代表有重复组员
            }
        }
        if(flag == 0){
            choosed = choosed + "," + realName;     //这是原版代码
    ...
    if (choosedId != null && choosedId.length > 0) {
//        choosedId = choosedId + "," + id;       //这是原版代码

        //改     Author：张赛超
        var flag = 0;              //flag=0代表无重复组员Id
        var temp = Array();
        temp = choosedId.split(',');
        for(var i=0; i<temp.length; i++){
            if(id == temp[i]){
                flag = 1;       //flag=1代表有重复组员Id
            }
        }
        if(flag == 0){
            choosedId = choosedId + "," + id;       //这是原版代码
...
//    $("#itemUserId").val("");
    $("select[name='hiddenType']>option:first").attr("selected","selected");
...
<t:formvalid formid="formobj" dialog="false" usePlugin="password" layout="table"
             action="tBHiddenDangerExamController.do?doAddAqdjc" tiptype="3" btnsub="btn" callback="callbackFun">
...
<tr>
    ...
    <td class="value">
        <input id="examDate" name="examDate" type="text" style="width: 150px" class="Wdate" datatype="*"
    ...
    <td class="value">
        <t:dictSelect field="shift" type="list" extendJson="{\"datatype\":\"*\"}"
                      typeGroupCode="workShift" defaultVal="${tBHiddenDangerExamPage.workShift}" hasLabel="false"
                      title="班次"></t:dictSelect>
...
<tr>
    ...
    <td class="value" colspan="3">
        <input id="teamMembers" type="text" name="teamMembers" style="width: 440px;" datatype="*"
               onclick="choose_fillCardMan('选择组员')"/>
        <input id="itemUserId" name="itemUserId" type="hidden" style="width: 150px" class="inputxt"
               value="" datatype="*">
        <a id="" class="easyui-linkbutton l-btn l-btn-plain" onclick="clearTeamMembers();" icon="icon-redo" plain="true"
           href="#">
            <span>清空</span>
        </a>
        <span class="Validform_checktip">请选择组员</span>
...
<tr>
    ...
    <td class="value">
        <t:dictSelect field="hiddenCategory" type="list" extendJson="{\"datatype\":\"*\"}"
                      typeGroupCode="hiddenCate" defaultVal="11"
                      hasLabel="false" title="隐患类别"></t:dictSelect>
    ...
    <td class="value">
        <t:dictSelect field="hiddenNature" type="list" extendJson="{\"datatype\":\"*\"}"
                      typeGroupCode="hiddenLevel" defaultVal="${tBHiddenDangerExamPage.hiddenNature}"
                      hasLabel="false" title="隐患等级"></t:dictSelect>
...
<tr>
    ...
    <td class="value" colspan="3">
        <label>
            <input type="radio" name="dealType" id="dealType_xianqi" value="1" checked="checked"
                   onclick="showTr('dealTypetr1');">限期整改
        </label>
        <label>
...
<tr class="xcclTR" style="display: none">
    ...
    <td class="value" colspan="3">
        <div id="reviewManNameSelect" style="width: 130px;height: 15px"></div>
            <%--<input id="reviewManName" name="reviewManName" type="text" style="width: 150px" class="inputxt" onclick="choose_reviewMan('复查人')">--%>
