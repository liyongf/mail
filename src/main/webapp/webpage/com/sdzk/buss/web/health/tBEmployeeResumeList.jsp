...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="tBEmployeeResumeList" onDblClick="dblClickDetail" checkbox="true" fitColumns="true" title="职工职业履历档案" actionUrl="tBEmployeeResumeController.do?datagrid" idField="id" fit="true" queryMode="group">
  <t:dgCol title="主键"  field="id"  hidden="true"  queryMode="single"  width="120" align="center"></t:dgCol>
   ...
   <t:dgCol title="岗位变更简历"  field="postChangeResume"    queryMode="single"  width="180" align="center"></t:dgCol>
   <t:dgCol title="是否删除"  field="isDelete"  hidden="true"  queryMode="single"  width="120" align="center"></t:dgCol>

   <t:dgToolBar title="编辑" operationCode="update" icon="icon-edit" url="tBEmployeeResumeController.do?goUpdate" funname="update"></t:dgToolBar>
   <t:dgToolBar title="查看" operationCode="detail" icon="icon-search" url="tBEmployeeResumeController.do?goUpdate" funname="detail"></t:dgToolBar>
 ...
 <script type="text/javascript">
     function dblClickDetail(rowIndex,rowData){
         var id=rowData.id;
         var url = "tBEmployeeResumeController.do?goUpdate&load=detail&id="+id;
         createdetailwindow("查看",url);
     }
 $(document).ready(function(){
 		//给时间控件加上样式
 			/* $("#tBEmployeeResumeListtb").find("input[name='createDate']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#tBEmployeeResumeListtb").find("input[name='updateDate']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#tBEmployeeResumeListtb").find("input[name='partiWorkDate']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});}); */
 });
 
//导入
function ImportXls() {
	openuploadwin('Excel导入', 'tBEmployeeResumeController.do?upload', "tBEmployeeResumeList");
}

//导出
function ExportXls() {
	JeecgExcelExport("tBEmployeeResumeController.do?exportXls","tBEmployeeResumeList");
}

//导出
function ExportXlsByT() {
	JeecgExcelExport("tBEmployeeResumeController.do?exportXlsByT","tBEmployeeResumeList");
}

//模板下载
function ImportXlsT() {
	JeecgExcelExport("tBEmployeeResumeController.do?importXlsT","tBEmployeeResumeList");
}
 </script>