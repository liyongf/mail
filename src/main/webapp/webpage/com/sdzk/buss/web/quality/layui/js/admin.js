...
layui.define(['layer'], function(exports) {
	...
	//导出表格
	admin.exportExcel = function(url, method, fileName, data) {
		var request = new XMLHttpRequest();
		request.open(method,baseServer + url, true);
		request.responseType = 'json'
		request.setRequestHeader('Cache-Control', 'no-cache');
		request.setRequestHeader("Content-type", "application/json");
		//request.setRequestHeader("ZKAH_TOKEN", setter.getToken().access_token);
		request.setRequestHeader("jsonData", data);
		request.onreadystatechange = function(res) {
			if (request.readyState == 4) {
				if (request.status == 200) {
					if (typeof saveFile === "function") {
						saveFile(base64ToUint8Array(request.response.datas), fileName)
					}
				}
			}
		}
	...
	admin.activeNav = function(url) {
		...
		if (url) {
			...
			if ($a.length > 0) {
				...
				$(headerDOM + '>.layui-nav>.layui-nav-item>a[nav-bind="' + $aUl.attr('nav-id') + '"]').parent().addClass(
					'layui-this');
			}
		} else {
		...
		admin.ajax({
			...
			success: function(res) {
				...
				if (tpl === true || tpl === 'true') {
					...
					res = admin.util.tpl(res, data, undefined, undefined);
				}
	...
	admin.ajax = function(param) {
		...
		param.success = function(result, status, xhr) {
			var before = admin.ajaxSuccessBefore(admin.parseJSON(result), param.url, {
				param: oldParam,
				reload: function(p) {
		...
		param.error = function(xhr, status) {
			if (xhr.responseJSON) {
				var before = admin.ajaxError(xhr.responseJSON);
			}
			//param.success({code: xhr.status, msg: xhr.statusText}, status, xhr);
		};
		// 解决缓存问题
		if (layui.cache.version && (!apiNoCache || param.dataType.toLowerCase() !== 'json')) {
	 ...
	 // ajax请求结束后的处理，返回false阻止代码执行
	 	admin.ajaxSuccessBefore  = function(res, url, obj) {
	       if (res) {
	           if (res.code == 201 || res.code == 207) {
	   ...
	   //ajax统一报错信息
	   admin.ajaxError = function(res) {
	       if (res) {
	           if (res.code == 201 || res.code == 207) {
	           } else {
	               layer.msg(res.msg, { icon: 2, anim: 6, time: 3500 }, function() {});
	               return false;
	           }
	           return true;
	       }
	...
	admin.putTempData = function(key, value, local) {
		var tableName = local ? tableName : tableName + '_tempData';
	...
	admin.btnLoading = function(elem, text, loading) {
		...
		if (loading) {
			...
			$elem.prepend(
				'<span class="ew-btn-loading-text"><i class="layui-icon layui-icon-loading layui-anim layui-anim-rotate layui-anim-loop"></i>' +
				text + '</span>');
	...
	admin.openCellAutoExpand = function() {
		...
		$body.off('mouseleave.openCellAutoExpand').on('mouseleave.openCellAutoExpand',
	...
	admin.resizeTable = function(time) {
		setTimeout(function() {
			var $tbView = pageTabs ? $(tabDOM + '>.layui-tab-content>.layui-tab-item.layui-show') : $(bodyDOM);
	...
	admin.events = {
		...
		psw: function() {
			...
			admin.strToWin(option.window).layui.admin.open($.extend({
				...
				title: '修改密码',
				shade: 0,
				url: option.url || 'components/tpl/password.html'
			}, admin.parseLayerOption(option)));
		},
		/* 退出登录 */
		logout: function() {
		...
		fullScreen: function() {
			...
			var $ti = $(this).find('i');
			var isFullscreen = document.fullscreenElement || document.msFullscreenElement || document.mozFullScreenElement ||
	...
	admin.chooseLocation = function(param) {
		...
		if (mapJsUrl === undefined) mapJsUrl =
			'https://webapi.amap.com/maps?v=1.4.14&key=006d995d433058322319fa797f2876f5';
		var isSelMove = false,
			selLocation;
		// 搜索附近
					...
					if (status === 'complete') {
						...
						for (var i = 0; i < pois.length; i++) {
							...
							if (poiItem.location !== undefined) {
								htmlList += '<div data-lng="' + poiItem.location.lng + '" data-lat="' + poiItem.location.lat +
									'" class="ew-map-select-search-list-item">';
								...
								htmlList +=
									'     <div class="ew-map-select-search-list-item-icon-ok layui-hide"><i class="layui-icon layui-icon-ok-circle"></i></div>';
								htmlList += '</div>';
							}
						}
						...
						if (result.tips) {
							...
							for (var i = 0; i < tips.length; i++) {
								...
								if (tipItem.location !== undefined) {
									htmlList += '<div data-lng="' + tipItem.location.lng + '" data-lat="' + tipItem.location.lat +
										'" class="ew-map-select-search-list-item">';
									htmlList +=
										'     <div class="ew-map-select-search-list-item-icon-search"><i class="layui-icon layui-icon-search"></i></div>';
	...
	admin.cropImg = function(param) {
		...
		layui.use(['Cropper', 'upload'], function() {
			...
			htmlStr += '             <img id="ew-crop-img" src="' + (imgSrc ? imgSrc : '') +
	...
	admin.util = {
		...
		animateNum: function(elem, isThd, delay, grain) {
			...
			$(elem).each(function() {
				...
				var startNum = 0.0,
					endNum = strNum;
				if (Math.abs(endNum * 1) > 10) startNum = parseFloat(int_dec[0].substring(0, int_dec[0].length - 1) + (
					int_dec[1] ? '.0' + int_dec[1] : ''));
				var oft = (endNum - startNum) / grain,
					temp = 0;
		...
		deepClone: function(obj) {
			...
			for (var key in obj) {
				...
				var copy = obj[key],
					cClass = admin.util.isClass(copy);
	...
	$(document).on('mouseenter', navItemDOM + ',' + navItemDOM + ' .layui-nav-child>dd', function() {
		if (admin.getPageWidth() > 768) {
			var $that = $(this),
				$navChild = $that.find('>.layui-nav-child');
	...
	var navHeaderItemDOM = '.layui-layout-admin >.layui-top-side .layui-nav .layui-nav-item';
	$(document).on('mouseenter', navHeaderItemDOM + ',' + navHeaderItemDOM + ' .layui-nav-child>dd', function() {
		if (admin.getPageWidth() > 768) {
			var $that = $(this),
				$navChild = $that.find('>.layui-nav-child');
			if ($navChild.length > 0) {
				...
				if (top < 100) {
					top += 50;
					$navChild.css('left', $that.offset().left);
				} else {
					$navChild.css('left', $that.offset().left + $that.outerWidth());
				}
				if (top + $navChild.outerHeight() > admin.getPageHeight()) {
	...
	}).on('mouseleave', navHeaderItemDOM + ',' + navHeaderItemDOM + ' .layui-nav-child>dd', function() {
		layer.closeAll('tips');
		var $this = $(this);
		$this.removeClass('admin-nav-hover');
		var $child = $this.find('>.layui-nav-child');
	...
	admin.tips = function(option) {
		layer.tips(option.text, option.elem, {
			...
			success: function(layero) {
				...
				var top = offset[0],
					left = offset.length > 1 ? offset[1] : undefined;
	...
	if (cache) {
		var keys = ['pageTabs', 'cacheTab', 'defaultTheme', 'navArrow', 'closeFooter', 'tabAutoRefresh', 'navPosition'];
