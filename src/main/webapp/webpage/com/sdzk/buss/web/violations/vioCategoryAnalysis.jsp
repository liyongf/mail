...
<script type="text/javascript">
    ...
    function tBThreeViolationsListsearch(){
        ...
        $('#jeecgEasyUIList').datagrid({
            ...
            fitColumns:true,
            showFooter:true,
            url:'tBThreeViolationsController.do?vioCategoryAnalysisDatagrid&isMonthQuery='+type+'&queryMonth='+queryMonth+'&startDate='+startDate+'&endDate='+endDate,
            toolbar: '#tb2',
        ...
        <table id="jeecgEasyUIList" >
            ...
            <tr>
                ...
                <c:forEach items="${list}" var="tsType">
                    <th field="vio_category${tsType.typecode}" width="30">${tsType.typename}</th>
                </c:forEach>
        ...
        <script type="text/javascript">
            ...
            function getData(data){
                ...
                for(var i=0; i<data.rows.length; i++){
                    ...
                    unitArray.push(str);*/
                    unitArray.push(deptname);
                    <c:forEach items="${list}" var="tsType" varStatus="status">
                    var vio_category${tsType.typecode} = map.get("${tsType.typecode}");
                    if(vio_category${tsType.typecode} == null){
                        vio_category${tsType.typecode} = new Array();
                        map.put("${tsType.typecode}",vio_category${tsType.typecode});
                    }
                    vio_category${tsType.typecode}.push(data.rows[i].vio_category${tsType.typecode});
                    </c:forEach>
                    rows.push({
                        belongMine: data.rows[i].units,
                        <c:forEach items="${list}" var="tsType" varStatus="status">
                        vio_category${tsType.typecode}: data.rows[i].vio_category${tsType.typecode} ,
                        </c:forEach>
            ...
            $('#jeecgEasyUIList').datagrid({
                ...
                fitColumns:true,
                showFooter:true,
                url:'tBThreeViolationsController.do?vioCategoryAnalysisDatagrid&isMonthQuery=${isMonthQuery}&queryMonth=${queryMonth}&startDate=${startDate}&endDate=${endDate}',
                toolbar: '#tb2',
                loadFilter: function(data){
                    return getData(data);
                }
