...
function fillFXDJFXZ(){
    var riskMay = $("#yeProbability").val();
    var riskLoss = $("#yeCost").val();
    ...
    if(riskMay!= null && riskMay != "" && riskLoss != null && riskLoss !=""){
        ...
        if(mut>=threshold_major) {
            ...
            document.getElementById("isMajor").checked=true;
            document.getElementById("notMajor").checked=false;
        }else{
            document.getElementById("isMajor").checked=false;
            document.getElementById("notMajor").checked=true;
            if(mut>=threshold_superior) {
                $("#yeRiskGrade").val("较大风险");
            }else{
                if(mut>=threshold_commonly) {
                    $("#yeRiskGrade").val("一般风险");
                }else{
                    $("#yeRiskGrade").val("低风险");
                }
            }
