...
<!-- 通用危险源 -->
<%--<div id="tempButtonColums" style="display: none">
    <div name="buttonColums" >
        <a class="easyui-linkbutton l-btn" href="#" iconcls="icon-reload" onclick="resizeWindow()">
            同步
        </a>
    </div>
</div>--%>
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="tBDangerSourceList" checkbox="true" pagination="true" fitColumns="false" title="专业通用危险源库" actionUrl="tBDangerSourceController.do?universalDangerSourceDatagrid" idField="id" fit="true" queryMode="group" sortName="isMajor,yeRecognizeTime" sortOrder="desc,desc">
      ...
      <t:dgToolBar title="录入" icon="icon-add" funname="goAddUniversalDangerSource" operationCode="add" ></t:dgToolBar>
      <t:dgToolBar title="编辑" icon="icon-edit" url="tBDangerSourceController.do?goUpdate" funname="goUpdateUniversalDangerSource" operationCode="update"></t:dgToolBar>
   <t:dgToolBar title="查看" icon="icon-search" url="tBDangerSourceController.do?goDetail&from=universalDangerList" funname="detail" width="850" height="450" operationCode="detail"></t:dgToolBar>
   <%--<t:dgToolBar title="导出" icon="icon-putout" funname="ExportXls" operationCode="export"></t:dgToolBar>--%>
   <t:dgToolBar title="同步" icon="icon-reload" funname="synchronization" operationCode="synchronize"></t:dgToolBar>
  </t:datagrid>
  </div>
 </div>
 ...
 window.top["reload_UniversalDangerSourceList"]=function(){
     $("#tBDangerSourceList").datagrid( "load");
 };
//导出
function ExportXls() {
	JeecgExcelExport("tBDangerSourceController.do?exportCommonRiskXls&lec=${initParam.les}","tBDangerSourceList");
}
 //同步数据
 ...
 function synchronization(){
     ...
     $.ajax({
         url : 'tBDangerSourceController.do?synchronizationDataRPC',
         type : 'post',
         cache : false,
  ...
  *跳转到编辑年度危险源页面
  */
 function goUpdateUniversalDangerSource(){
     //获取选中条目
     ...
     if(rowsData == null || rowsData.length != 1){
         ...
         addOneTab("专业通用危险源编辑","tBDangerSourceController.do?goUpdateUniversalDangerSource&id="+id,"default");
     }
 }

 function goAddUniversalDangerSource(){
     addOneTab("专业通用危险源录入","tBDangerSourceController.do?goAddUniversalDangerSource","default");
 }
 </script>