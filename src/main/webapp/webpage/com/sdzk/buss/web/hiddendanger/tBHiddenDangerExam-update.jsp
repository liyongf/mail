...
<head>
	...
	<script type="text/javascript">
		...
		$(function() {
            ...
            fillKeyWords();
			getUserMagicSuggestWithValue($("#reviewManNameSelect"), $("input[name='reviewMan.id']"),"${tBHiddenDangerExamPage.reviewMan.id}",false);
...
<t:formvalid formid="formobj" dialog="false" usePlugin="password" layout="table" action="tBHiddenDangerExamController.do?doUpdate" tiptype="3" btnsub="btn" callback="callbackFun">
    ...
    <tr>
		...
		<tr>
			...
			<td class="value">
				<input id="examDate" name="examDate" type="text" style="width: 150px"  datatype="*" class="Wdate" onClick="WdatePicker({maxDate:'%y-%M-%d'})" value='<fmt:formatDate value='${tBHiddenDangerExamPage.examDate}' type="date" pattern="yyyy-MM-dd"/>'>
			...
			<td class="value">
                <%--<t:dictSelect field="shift" type="list"--%>
                              <%--dictTable="t_b_shift_manage" dictField="id" dictText="shift_name"--%>
                              <%--defaultVal="${tBHiddenDangerExamPage.shift}" hasLabel="false" dictCondition=" where is_used = 1 "--%>
		...
		<c:if test="${examType eq 'sjjc'}">
			...
			<tr>
				<td align="right">
					<label class="Validform_label">
						<font color="red">*</font>上级检查部门:
					</label>
				</td>
				<td class="value">
					<input id="sjjcDept" name="sjjcDept" type="text" style="width: 150px" class="inputxt" datatype="*" value="${tBHiddenDangerExamPage.sjjcDept}" >
					<span class="Validform_checktip">请填写上级检查部门</span>
					<label class="Validform_label" style="display: none;">上级检查部门</label>
				</td>
				<td align="right">
					<label class="Validform_label">
						检查人:
				...
				<td class="value">
					<input id="sjjcCheckMan" name="sjjcCheckMan" type="text" style="width: 150px" class="inputxt" value="${tBHiddenDangerExamPage.sjjcCheckMan}" >
					<span class="Validform_checktip"></span>
					<label class="Validform_label" style="display: none;">检查人</label>
				</td>
			</tr>
        ...
        <c:if test="${examType eq 'other'}">
			<tr>
				<td align="right">
					<label class="Validform_label">
						<font color="red">*</font>检查类型:
					</label>
				</td>
				<td class="value" colspan="3">
					<input id="checkType" name="checkType" type="text" style="width: 150px" class="inputxt" datatype="*" value="${tBHiddenDangerExamPage.checkType}" >
					<span class="Validform_checktip">请填写检查类型</span>
					<label class="Validform_label" style="display: none;">检查类型</label>
				</td>
			</tr>
		</c:if>
        ...
        <c:if test="${examType eq 'sjjc'}">
            <tr>
                ...
                <td class="value" colspan="3">
                    <textarea id="remark" name="remark" type="text" style="width: 440px;"  value="">${tBHiddenDangerExamPage.remark}</textarea>
                </td>
            </tr>
        </c:if>
		<tr>
		...
		<c:if test="${examType eq 'sjjc'}">
			<tr >
				...
				<td class="value" colspan="3">
					...
					<input id="reviewManId" name="reviewMan.id"  style="width: 150px" class="inputxt" type ="hidden"  datatype="*" value="${tBHiddenDangerExamPage.reviewMan.id}" >
					<span class="Validform_checktip"></span>
