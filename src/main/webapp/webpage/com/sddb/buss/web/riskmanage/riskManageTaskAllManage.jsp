...
<html>
  ...
  <t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" action="riskManageTaskAllManageController.do?save" beforeSubmit="showWait" callback="@Override hideWait">
			...
			<table style="width: 100%;" cellpadding="0" cellspacing="1" class="formtable" id="tab">
				<tr>
					...
					<td class="value">
						<input class="Wdate" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})"  style="width: 150px" id="manageTime" name="manageTime"  datatype="*"   value="<fmt:formatDate value='<%=new Date()%>' type="date" pattern="yyyy-MM-dd"/>" />
					...
					<td class="value">
						<c:if test="${hegang ne 'true'}">
							<input class="inputxt" id="manageName" name="manageName"  datatype="*"  value="${riskManageTaskAllManagePage.manageName}" />
						</c:if>
						<c:if test="${hegang eq 'true'}">
							<div id="manageType1">
								<input class="inputxt" id="manageName" name="manageName" datatype="*"   value="${riskManageTaskAllManagePage.manageName}" />
							</div>
							<div id="manageType2" style="display: none">
								<t:dictSelect field="manageNameTemp" type="list"
											  typeGroupCode="manageNameTemp" defaultVal="${riskManageTaskAllManagePage.manageName}" hasLabel="false"  title="管控名称"></t:dictSelect>
							</div>
						</c:if>
						<span class="Validform_checktip"></span>
						<label class="Validform_label" style="display: none;">管控名称</label>
					</td>
				</tr>
				<tr>
					<td align="right">
					...
					<td class="value">
						<input class="Wdate" onClick="WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'%y-%M-%d'})"  style="width: 150px" id="endDate" name="endDate" datatype="*"    value="<fmt:formatDate value='${riskManageTaskAllManagePage.endDate}' type="date" pattern="yyyy-MM-dd"/>" />
						<span class="Validform_checktip"></span>
					...
					<td class="value">
						<textarea id="mainContents" name="mainContents" type="text" style="width: 440px;"  datatype="*" >${riskManageTaskAllManagePage.mainContents}</textarea>
						<span class="Validform_checktip"></span>
					</td>
				</tr>
				<tr>
 ...
 <script type="text/javascript">
     ...
     $(function () {
		 ...
		 if(${hegang eq 'true'}){
			 $("[name='manageType']").trigger("change");
		 }

		 $("[name='manageType']").change (function () {
			 if(${hegang eq 'true'}){
				 if($("[name='manageType']").val()=='team'){
					 $("#manageName").removeAttr("datatype");
					 $("#manageName").val("");
					 $("[name='manageNameTemp']").attr("datatype","*");
					 $("#manageType1").hide();
					 $("#manageType2").show();
				 } else{
					 $("#manageName").attr("datatype","*");
					 $("[name='manageNameTemp']").removeAttr("datatype");
					 $("[name='manageNameTemp']").val("");
					 $("#manageType1").show();
					 $("#manageType2").hide();
				 }
			 }
