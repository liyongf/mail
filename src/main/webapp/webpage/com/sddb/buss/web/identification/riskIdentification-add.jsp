    ...
    <div region="center" style="padding:0px;border:0px" >
        ...
        <t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" action="riskIdentificationController.do?doAdd" >
                    ...
                    <c:if test="${identificationType ne '4'}">
                        ...
                        <td class="value">
                            ...
                            <div class="ui_buttons" style="text-align: left;display: inline">
                                <c:if test="${identificationType ne '3' && identificationType ne '5' && riskManageTaskAllId eq ''}">
                                <input type="button" id="btn" value="克隆已辨识风险点" onclick="showAddress();" class="ui_state_highlight" >
                                </c:if>
                                <c:if test="${identificationType eq '3' || identificationType eq '5'}">
                                    <input type="button" id="btn" value="克隆已辨识风险" onclick="showAddress();" class="ui_state_highlight" >
                                </c:if>
                            </div>
            ...
            <div class="easyui-layout" fit="true"  id="main_typegroup_list">
                ...
                <t:datagrid name="hazardFactorsList" checkbox="true" pagination="true"  onClick="toModify" fitColumns="true" title="" height="" onLoadSuccess="dataGridLoadSuccess" actionUrl="riskIdentificationController.do?datagrid&identificationType=${identificationType}&load=${load}&riskTaskId=${riskTaskId}&riskManageTaskId=${riskManageTaskId}&riskManageTaskAllId=${riskManageTaskAllId}&riskManageTaskAllIdYH=${riskManageTaskAllIdYH}&id=${id}" idField="id" fit="true" queryMode="group" sortName="createDate" sortOrder="desc">
                    ...
                    <t:dgCol title="危害因素和管控措施" align="center"  field="hazardFactortsNum" url="riskIdentificationController.do?wxysList&id={id}&load=${load}" queryMode="group"  width="120" ></t:dgCol>
                    ...
                    <c:if test="${load ne 'detail'}">
                        ...
                        <t:dgToolBar title="批量编辑" icon="icon-edit" url="riskIdentificationController.do?goAllEdit" funname="goAllEdit"></t:dgToolBar>
                    </c:if>
                </t:datagrid>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    function  showAddress() {
        ...
        if(identificationType != '4'){
            if($("#address").val().length >0){
                openwindow("克隆风险点","riskIdentificationController.do?goClone&id="+$("#address").val()+"&identificationType=${identificationType}&riskTaskId=${riskTaskId}&riskManageTaskId=${riskManageTaskId}","",700,400);
            }else{
                tip("请先选择风险点");
            }
        }else{
            var postId = document.getElementById("post.id").value;
            if(postId.length >0){
                openwindow("岗位风险点","riskIdentificationController.do?goClone&id="+postId+"&identificationType=${identificationType}&riskTaskId=${riskTaskId}&riskManageTaskId=${riskManageTaskId}","",700,400);
            }else{
                tip("请先选择岗位");
            }
        }
        ...
        if(rows ==null || rows.length <=0){
            ...
            $.ajax({
                ...
                success : function(data) {
                    ...
                    if (d=="1") {
                        openwindow("提交审批", "riskIdentificationController.do?goReport&ids=" + ids+"&identifiManId="+identifiManId, "hazardFactorsList", 600, 450);
                        /*$.ajax({
    ...
    $(function() {
        ...
        if(addressId.length >0) {
            magicsuggestSelected = $('#magicsuggest').magicSuggest({
                data: 'magicSelectController.do?getAddressList&addressId=${addressId}',
        ...
        $(magicsuggestSelected).on('selectionchange', function(e,m){
            ...
            if(firstLoad){
                //查询上次辨识人
                $.ajax({
                    url : "riskIdentificationController.do?getLastIdentifiMan&load=${load}&id=${id}",
                    type : 'post',
                    cache : false,
                    data : {
                        address: $("#address").val(),
                    ...
                    success : function(data) {
                        setTimeout(userSetValue,500);
                        function userSetValue() {
                            userSelected.setValue(eval(data));
                        }
                    }
                });
    ...
    function addRisk(riskType){
        ...
        if(identificationType == "2" || identificationType == '3' || identificationType == '5'|| identificationType == '6'
        ...
        $.ajax({
            ...
            data : {
                ...
                riskManagePostHazardFactorId:'${riskManagePostHazardFactorId}',
                specificType:specificType,
                specificName:specificName,
                riskTaskId:'${riskTaskId}',
                riskManageTaskId:'${riskManageTaskId}',
                riskManageTaskAllIdYH:'${riskManageTaskAllIdYH}'
            },
            success : function(data) {
                var d = $.parseJSON(data);
                if (d.success) {
    ...
    function goAllEdit() {
        ...
        if (rows.length == 0) {
            ...
            $.dialog.confirm("已勾选"+idsTemp.length+"条记录,是否确认批量编辑？", function () {
                openwindow("批量编辑", "riskIdentificationController.do?goAllEdit&ids=" + idt, "hazardFactorsList", 750, 400);
            });
        }
    }
</script>