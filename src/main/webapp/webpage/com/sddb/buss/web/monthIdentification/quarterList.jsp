...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="quarterRiskList" onDblClick="dblClickDetail" checkbox="true" fitColumns="false" title="月度风险管控清单" actionUrl="monthRiskIdentificationController.do?datagrid&type=${type}" idField="id" fit="true" queryMode="group" >
      <t:dgCol title="主键"  field="id"  hidden="true"  queryMode="group"  width="120"></t:dgCol>
      <t:dgCol title="年份"  field="month"   width="150" align="center"></t:dgCol>
      <t:dgCol title="年份" field="yeRecognizeTimeTemp" query="true" formatter="yyyy" queryMode="single" hidden="true" width="90" align="center"></t:dgCol>
      <t:dgCol title="季度" field="quarter" query="true"  dictionary="quarter"  width="90" align="center"></t:dgCol>
 ...
 <script type="text/javascript">
     ...
     //导出
     function ExportXls() {
         JeecgExcelExport("monthRiskIdentificationController.do?exportXls&type=${type}","quarterRiskList");
     }

     //导入
     function ImportXls() {
         openuploadwin('月度风险管控清单', 'monthRiskIdentificationController.do?upload&type=${type}', "quarterRiskList");
     }
     //模板下载
     function ExportXlsByT() {
         JeecgExcelExport("monthRiskIdentificationController.do?exportXlsByT&type=${type}","quarterRiskList");
     }
     ...
     function goDetail(){
         //获取选中条目
         var rowsData = $('#quarterRiskList').datagrid('getSelections');
     ...
     function deleteALLSelect(title,url,gname) {
         gridname=gname;
         var ids = [];
         var rows = $("#quarterRiskList").datagrid('getSelections');
         if (rows.length > 0) {
 ...
 $(document).ready(function(){
     ...
     var datagrid = $("#quarterRiskListtb");
     ...
     datagrid.find("form[id='quarterRiskListForm']>span:first").before($("#tempSearchColums div[name='addressSearchColums']").html());
     $("#tempSearchColums div[name='addressSearchColums']").remove();

     getAddressMagicSuggest($("#addressSelect"), $("[name='addressId']"));
     $("#quarterRiskListtb").find("input[name='yeRecognizeTimeTemp']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy'});});
     $("[name='yeRecognizeTimeTemp']").val('${year}');
     $(".icon-reload").bind("click", function(){
         window.location.reload();
     });
 });

         window.top["reload_quarterRiskListtb"]=function(){
         $("#quarterRiskList").datagrid( "load");
