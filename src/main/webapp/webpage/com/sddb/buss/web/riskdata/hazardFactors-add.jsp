...
<html>
 <head>
  ...
  <t:base type="jquery,easyui,tools,DatePicker"></t:base>
	 ...
	 <script type="text/javascript">
         ...
         function refresh(){
             var taskManageId =  $("#taskManageId").val();
             if(taskManageId!=''&&taskManageId!=null&&taskManageId!=undefined){
 ...
 <body>
  <t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" action="hazardFactorsController.do?doAdd" callback="@Override refresh">
		<input id="id" name="id" type="hidden" value="${hazardFactorsEntityPage.id }"/>
	  <input id="taskManageId" name="taskManageId" type="hidden" value="${taskManageId }"/>
		<table style="width: 600px;" cellpadding="0" cellspacing="1" class="formtable">
			...
			<tr>
				...
				<td class="value">
					<textarea  id="hazardFactors" name="hazardFactors" cols="6" style="width: 80%" <c:if test="${jinqiao eq 'true'}">datatype="*2-500"</c:if> <c:if test="${jinqiao ne 'true'}">datatype="*2-200"</c:if> >${hazardFactorsEntityPage.hazardFactors}</textarea>
					<span class="Validform_checktip"></span>
