...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="riskIdentificationPostList" checkbox="true" pagination="true" fitColumns="true" onDblClick="dblClickDetail" title="岗位风险辨识" actionUrl="riskIdentificationController.do?postDatagrid&riskTaskId=${riskTaskId}" idField="id" fit="true" queryMode="group">
   ...
   <t:dgCol title="危害因素"  align="center" field="hazardFactortsPostNum" url="riskIdentificationController.do?wxysPostList&id={id}"  queryMode="group"  width="120" ></t:dgCol>
      <c:if test="${detail ne 'true'}">
          <t:dgToolBar title="批量删除"  icon="icon-remove" url="riskIdentificationController.do?doBatchDelPost" funname="deleteALLSelect"></t:dgToolBar>
          <%--<t:dgToolBar title="查看" icon="icon-search" url="riskIdentificationController.do?goDetail" funname="godetail"></t:dgToolBar>--%>
          <t:dgToolBar title="导入" icon="icon-put" funname="ImportXlsPost"></t:dgToolBar>
          <t:dgToolBar title="模板下载" icon="icon-putout" funname="ExportXlsByTPost"></t:dgToolBar>
      </c:if>
 ...
 <script type="text/javascript">
     window.top["reload_riskIdentificationPostList"]=function(){
         riskIdentificationPostListsearch();
     };
 ...
 $(document).ready(function(){
     ...
     //导入
     function ImportXlsPost() {
         openuploadwin('岗位风险导入', 'riskIdentificationController.do?upload&riskTaskId=${riskTaskId}', "riskIdentificationPostList");
     }

     //模板下载
     function ExportXlsByTPost() {
         JeecgExcelExport("riskIdentificationController.do?exportXlsByT","riskIdentificationPostList");
     }
     ...
     function ExportXls() {
         var rows = $("#riskIdentificationPostList").datagrid('getSelections');
         if (rows.length == 0) {
             JeecgExcelExport("riskIdentificationController.do?exportXlsPost", "riskIdentificationPostList");
             ...
             $.dialog.confirm("是否确认导出"+idsTemp.length+"条记录？", function () {
                 JeecgExcelExport("riskIdentificationController.do?exportXlsPost&riskTaskId=${riskTaskId}&ids="+idt, "riskIdentificationPostList");
             });
         }
     }
