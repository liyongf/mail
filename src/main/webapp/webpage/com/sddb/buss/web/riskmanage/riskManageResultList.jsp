...
<div class="easyui-layout" fit="true" id="main_typegroup_list">
    <div region="center" style="padding:0px;border:0px">
        <t:datagrid name="riskManageResultList" checkbox="true" pagination="true" fitColumns="true" title="${riskManageName}结果" actionUrl="riskManageResultController.do?relRiskDatagrid&manageType=${manageType}&taskAllId=${taskAllId}" idField="id" fit="true" queryMode="group">
            ...
            <t:dgToolBar title="删除"  icon="icon-remove" url="riskManageTaskController.do?doBatchDel&taskAllId=${taskAllId}" funname="deleteALLSelect"  ></t:dgToolBar>
            <t:dgToolBar title="查看风险" icon="icon-search"  funname="detailOneself" ></t:dgToolBar>
            <t:dgCol title="操作" field="opt" width="200" align="center"></t:dgCol>
            <t:dgFunOpt funname="showHazardFactorsList(id)" title="录入"  urlclass="ace_button" ></t:dgFunOpt>
        </t:datagrid>
    </div>
</div>
<div data-options="region:'east',
	...
	onCollapse : function() {
     ...
     style="width: 900px; overflow: hidden;" id="eastPanel">
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        var datagrid = $("#riskManageResultListtb");
        datagrid.find("div[name='searchColums']").append($("#tempSearchColums div[name='searchColums']").html());
        $("a[iconcls='icon-reload']").hide();
        <c:if test="${manageType ne 'post'}">
        datagrid.find("form[id='riskManageResultListForm']>span:first").before($("#tempSearchColums div[name='addressSearchColums']").html());
        $("#tempSearchColums div[name='addressSearchColums']").remove();
        getAddressMagicSuggest($("#addressSelect"), $("[name='addressId']"));
        </c:if>
        <c:if test="${manageType eq 'post'}">
        datagrid.find("form[id='riskManageResultListForm']>span:first").before($("#tempSearchColums div[name='postSearchColums']").html());
        ...
        $("input[type=radio][name='queryHandleStatusTem']").change(function() {
            ...
            riskManageResultListsearch();
        });
        datagrid.find("form[id='riskManageResultListForm'] span [name='risk.dutyManager']").prev().css("width","100");
    ...
    function reloadP(){
        $('#riskManageResultList').datagrid('reload');
    }


    function detailOneself(title,url, id,width,height) {
    ...
    function showHazardFactorsList(id){
        ...
        $('#main_typegroup_list').layout('panel','east').panel('setTitle', "危害因素列表");
        $('#hazardFactorListpanel').panel("refresh", "riskManageResultController.do?hazardFactorList&load=add&taskManageId="+id);
    }
    ...
    window.top["reload_riskManageResultList"]=function(){
        $("#riskManageResultList").datagrid("reload");
        riskManageResultListsearch();
